...
function adjustDiceAfterGame(winner) {
  ...
  for (let i = 0; i < agentMoves.length; i++) {
    ...
    if (newDieType) {
      adjustFn(pos, idx);
      // If not the first move, also adjust the subsequent move's die
      if (i > 0 && i + 1 < agentMoves.length) {
        const { pos: nextPos, idx: nextIdx } = agentMoves[i + 1];
        const nextDieType = targetDieTypeFn(`D${diceMatrix[nextPos][nextIdx]}`);
        if (nextDieType) {
          adjustFn(nextPos, nextIdx);
        }
      }
  ...
  if (isAgentWinner && agentMoves[agentMoves.length - 1].pos === 0 && !trainingDoneOnce) {
    trainingDone = true;
    trainingDoneOnce = true;
